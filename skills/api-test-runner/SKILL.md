...
```bash
mkdir -p setup
cp -R "$AGENT_KIT_HOME/skills/api-test-runner/template/setup/api" setup/
```

Bootstrap a runnable public-endpoint smoke suite (includes `setup/api`, `setup/rest`, `setup/graphql`):

```bash
cp -R "$AGENT_KIT_HOME/skills/api-test-runner/template/setup" .
```

Run a canonical suite:

```bash
$AGENT_KIT_HOME/skills/api-test-runner/scripts/api-test.sh --suite smoke-demo --out out/api-test-runner/results.json
```

Emit JUnit for CI reporters:

```bash
$AGENT_KIT_HOME/skills/api-test-runner/scripts/api-test.sh --suite smoke-demo --junit out/api-test-runner/junit.xml
```

Generate a human-friendly summary (CI logs + `$GITHUB_STEP_SUMMARY`), based on the results JSON:

```bash
$AGENT_KIT_HOME/skills/api-test-runner/scripts/api-test-summary.sh \
...
```bash
$AGENT_KIT_HOME/skills/api-test-runner/scripts/api-test.sh \
  --suite smoke \
...
steps:
  - name: Run suite shard
    env:
      AGENT_KIT_HOME: ${{ github.workspace }}
