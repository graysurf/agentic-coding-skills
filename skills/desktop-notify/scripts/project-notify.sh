...
Environment:
  ...
  AGENT_KIT_DESKTOP_NOTIFY_HINTS=1  Print install hints when backend missing (default: disabled)
EOF
}
...
if [[ -n "${AGENT_KIT_HOME:-}" ]]; then
  desktop_notify_candidates+=("${AGENT_KIT_HOME%/}/skills/desktop-notify/scripts/desktop-notify.sh")
  desktop_notify_candidates+=("${AGENT_KIT_HOME%/}/scripts/desktop-notify.sh")
fi
