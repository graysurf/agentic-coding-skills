...
description: Open files edited by the agent in VSCode after making changes, using the bundled open-changed-files script (silent no-op when unavailable).
---
...
- `$AGENT_KIT_HOME/skills/open-changed-files-review/scripts/open-changed-files.zsh` available.

Inputs:

- File paths touched in the current run (preferred).
- Optional: `OPEN_CHANGED_FILES_MAX_FILES` to cap opened files.
...
2. Determine the cap:
   - Default: `OPEN_CHANGED_FILES_MAX_FILES=50`
