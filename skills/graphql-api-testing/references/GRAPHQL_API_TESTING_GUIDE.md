...
```bash
cp "$AGENT_KIT_HOME/skills/graphql-api-testing/references/GRAPHQL_API_TESTING_GUIDE.md" \
  "docs/backend/graphql-api-testing-guide.md"
```
...
To initialize `setup/graphql/` in a repo for the first time, copy the bundled template from this kit:
...
```bash
$AGENT_KIT_HOME/skills/graphql-api-testing/scripts/gql-schema.sh --config-dir setup/graphql --cat
```
...
6) Call GraphQL operations (recommended: bundled script)
...
```bash
$AGENT_KIT_HOME/skills/graphql-api-testing/scripts/gql.sh --list-envs
```

List JWT profiles:

```bash
$AGENT_KIT_HOME/skills/graphql-api-testing/scripts/gql.sh --list-jwts
```
...
- Prefer “files + template command” (or `$AGENT_KIT_HOME/skills/graphql-api-testing/scripts/gql.sh`) over ad-hoc one-liners: it reduces drift and quoting mistakes.
