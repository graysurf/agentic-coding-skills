...
[[ -n "$script_dir" ]] || die "Unable to resolve script directory"

default_agent_kit_home="$(cd "${script_dir%/}/.." >/dev/null 2>&1 && pwd -P || true)"
AGENT_KIT_HOME="${AGENT_KIT_HOME:-$default_agent_kit_home}"
[[ -n "$AGENT_KIT_HOME" ]] || die "AGENT_KIT_HOME is required (unable to resolve default)"

USER_DATA_DIR_BASE="${CHROME_DEVTOOLS_USER_DATA_BASE:-${AGENT_KIT_HOME%/}/out/chrome-devtools-mcp}"
...
# 1) Load .env (default: $AGENT_KIT_HOME/.env) into environment
ENV_FILE="${ENV_FILE:-${AGENT_KIT_HOME%/}/.env}"
if [[ -f "$ENV_FILE" ]]; then
  set -a
...
# Log setup (default to $AGENT_KIT_HOME/out)
# IMPORTANT: Do not redirect stdout; MCP uses stdout for the protocol.
LOG_DIR="${CHROME_DEVTOOLS_LOG_DIR:-${AGENT_KIT_HOME%/}/out}"
