...
USER_DATA_DIR_BASE="${CHROME_DEVTOOLS_USER_DATA_BASE:-~/chrome-devtools/.cache/chrome-devtools-mcp}"
...
# 1) Load .env (default: ~/chrome-devtools/.env) into environment
ENV_FILE="${ENV_FILE:-~/chrome-devtools/.env}"
if [[ -f "$ENV_FILE" ]]; then
  set -a
...
# Log setup (default to ~/.chrome-devtools/output)
# IMPORTANT: Do not redirect stdout; MCP uses stdout for the protocol.
LOG_DIR="${CHROME_DEVTOOLS_LOG_DIR:-~/chrome-devtools/output}"
